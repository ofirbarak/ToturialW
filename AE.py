...
def encoder(images, bs):
    ...
    buffers = conv(INPUT_SIZE, NLAYER_SIZES[0], buffers, None, kernels[0], encbiases[0],
                   STRIDE[0], WINDOW[0], trans, K[0], 'Enc' + str(0))
    layer_buffers.append(buffers)
    # print('endcoder second layer')
    for i in range(1, NUM_LAYERS):
        buffers, loc = conv(NLAYER_SIZES[i-1], NLAYER_SIZES[i], buffers, None, kernels[i], encbiases[i],
...
def decoder(foward_buffers):
    trans = True
    print(len(foward_buffers))
    zbuffers = foward_buffers[-1]

    for i in range(NUM_LAYERS-1, 0, -1):
        zbuffers = conv(NLAYER_SIZES[i], NLAYER_SIZES[i-1], zbuffers, foward_buffers[i], kernels[i], decbiases[i],
                        STRIDE[i], WINDOW[i], trans, K[i-1], 'Dec'+str(i))
    # print('decoder first layer')
    zbuffers = conv(NLAYER_SIZES[0], INPUT_SIZE, zbuffers, foward_buffers[0], kernels[0], decbiases[0],
